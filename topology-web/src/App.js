...
function App() {
  ...
  useEffect(() => {
    axios.get("http://localhost:8000/api/topology")
      .then(res => drawGraph(res.data.nodes, res.data.links));
  ...
  return (
    <div style={{ display: "flex", flexDirection: "row" }}>
      <div style={{ margin: "20px" }}>
        ...
        {tooltip.visible && (
          <div style={{ position: "absolute", left: tooltip.x + 15, top: tooltip.y + 15,
            backgroundColor: "rgba(0,0,0,0.7)", color: "#fff", padding: "5px", borderRadius: "4px", pointerEvents: "none" }}>
            {tooltip.text}
          </div>
        )}
      </div>
                  ...
                  <tbody>
                    {selectedDevice.interfaces?.map((iface, i) => (
                      <tr key={i}>
                        ...
                        <td>{iface.ip}</td>
                        <td>
                          <span style={{ color: iface.status === "up" ? "green" : "red", fontWeight: "bold" }}>
                            {iface.status}
                          </span>
                        </td>
                        <td>
                          <span style={{ color: iface.protocol === "up" ? "green" : "red", fontWeight: "bold" }}>
                            {iface.protocol}
                          </span>
                        </td>
                      </tr>
