...
export default function IDSPage() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case "미해결":
        return (
          <span className="rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-800 border border-red-300">
            미해결
          </span>
        );
      case "조사 중":
        return (
          <span className="rounded-full bg-yellow-100 px-2 py-1 text-xs font-semibold text-yellow-800 border border-yellow-300">
            조사 중
          </span>
        );
      case "해결됨":
        return (
          <span className="rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-800 border border-green-300">
            해결됨
          </span>
        );
      default:
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">시간</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">유형</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">출발지</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">목적지</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">심각도</th>
                    <th scope="col" className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">상태</th>
                    <th scope="col" className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">작업</th>
                  </tr>
                </thead>
                  ...
                  {filteredAlerts.length > 0 ? (
                    filteredAlerts.map((alert) => (
                      <tr key={alert.id} className="hover:bg-gray-50">
                        <td className="whitespace-nowrap px-4 py-4 text-sm font-medium text-gray-900">{alert.id}</td>
                        <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-500">{alert.timestamp}</td>
                        <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-500">{alert.type}</td>
                        <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-500">{alert.source}</td>
                        <td className="whitespace-nowrap px-4 py-4 text-sm text-gray-500">{alert.destination}</td>
